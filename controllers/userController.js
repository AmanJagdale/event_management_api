    ...
    .on('end', async () => {
      try {
        ...
        for (const row of results) {
          const { full_name, email, password, role, bio, phone } = row;
          const name = full_name || row.name; // Fallback to 'name' if 'full_name' is missing
          ...
          await db.query(
            ...
            [name, email, hashedPassword, role || 'member', bio || null, phone || null]
          );
          successCount++;
        }
